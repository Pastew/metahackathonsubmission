...
using System.Collections;
    ...
    public class ETFXProjectileScript : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            projectileParticle.transform.localScale = Vector3.one * scale;
            
            if (muzzleParticle)
            {
                muzzleParticle = Instantiate(muzzleParticle, transform.position, transform.rotation) as GameObject;
                muzzleParticle.transform.localScale = Vector3.one * scale;
        ...
        {	
            ...
            if (Physics.SphereCast(transform.position, radius, direction, out hit, detectionDistance)) // Checks if collision will happen
            {
                ...
                impactP.transform.localScale = Vector3.one * scale;
